...
const HIRING_FILTER_LABELS: Record<HiringFilter, string> = {
  any: "hiring or not",
  hiring: "has open roles",
  no_hiring: "no job postings",
};
...
export function DiscoveryReviewList({
  ...
  const filterLabels = activeReviewFilters(active).map((filter) => {
    ...
    if (filter.key === "hiring") {
      return HIRING_FILTER_LABELS[filter.value as HiringFilter];
    }
