 ...
 * queue while the list counted a filtered slice of it. Every predicate below
 * mirrors one in getReviewQueue's buildConditions and must stay in step; the
 * chip for the bucket currently on screen is additionally driven by the list's
 * own totalMatched, so any drift surfaces as a wrong number on an inactive chip
 * rather than as an empty list under a non-zero count.
 */
export async function getReviewBucketCounts(scope: {
  ...
  if (scope.vertical) {
    conditions.push(sql`${companies.vertical} = ${scope.vertical}`);
  }
  if (scope.market) {
    conditions.push(sql`${companies.sourceMarket} = ${scope.market}`);
  }
  const term = scope.search?.trim();
  if (term) {
    const pattern = `%${escapeLike(term)}%`;
    conditions.push(sql`(
      ${companies.name} ILIKE ${pattern}
